<?php 
	...
	function generatePhotoName($ext)
	{
		...
		$temp_name = 'Villa-form-provisioning-'.$date.".$ext";//.'-'.$_FILES['img']['name'];//"Villa-form-Inspiringvilla-$randomString.$ext";//$randomString.$ext";
	...
	if (isset($_FILES['file_provisioning']) && !empty($_FILES['file_provisioning']['name'])) {
		$file_ext = pathinfo($_FILES['file_provisioning']['name'], PATHINFO_EXTENSION);
		...
		if (move_uploaded_file($_FILES['file_provisioning']['tmp_name'], $path)) {
