<?php
	...
	if ( $_REQUEST['search']['value'] != "" ){
		$sWhere = "WHERE p.destination = '" . $_GET['destinationID'] . "' $condition_beach AND (  ";
	...
	{
		$sWhere .="WHERE p.destination = '" . $_GET['destinationID'] . "' $condition_beach ";
	}
	...
	while ( $aRow = $dbc->Fetch( $rResult ) ){
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ ){
			if($i==1)
			{
				$rower = json_decode($aRow[$i],true);
				//echo '----------------'.print_r($rower[0]['img']);
				if($rower[0]['img']=='')
				{
					$row[] = '0';
				}
				...
				{
                    //re-write the image URL
                    $rower[0]['img'] = imagePath( $rower[0]['img'] );
					$row[] = $rower[0];
				}
