...
switch($page)
{
	...
	case"villa-promotions":
		$url = "villa-promotions";
	break;
	case"propertydetail":
		$ca_prop = $dbc->GetRecord("properties","*","id = '".$_REQUEST['id']."' ");
		$photo_main = json_decode($ca_prop['photo'],true);
		$photo_id = $photo_main[0]['img'];
		//echo '>>>'.S3_BUCKET_URL.$photo_id;
		echo '<link href="'.imagePath($photo_main[0]['img']).'" rel="me"/>';
	...
	case"yacht":
		...
		{
			$ca_prop = $dbc->GetRecord("yacht","*","id = '".$_REQUEST['id']."' ");
			$photo_main = json_decode($ca_prop['photo'],true);
			$photo_id = 'https://www.inspiringvillas.com/'.$photo_main;
			//echo '>>>'.S3_BUCKET_URL.$photo_id;
			echo '<link href="'.imagePath($photo_main).'" rel="me"/>';
			//echo $ca_prop['ht_link'];
		...
		else
		{
			...
			$url = "yacht";
			$photo_id = 'https://www.inspiringvillas.com/'.$photo;
