...
<div class="popbox">
    ...
    <div class="row nopad">
        ...
        <div class="col-sm-11 t_t222 " style="margin-bottom:15px;">
        	<input type="hidden" class="txs_link">
            <input type="hidden" class="txs_name">
            <input type="hidden" class="txs_shortdescription">
            <input type="hidden" class="txs_id">
...
<div class="box_share_bg" onClick="closepopup2();"></div>
<div class="box_share">
	<div class="box_email">
    	<div class="row">
        	<div class="col-md-12"><strong>Enter Your <span class="textsh">...</span></strong></div>
        	<div class="col-md-9"><select name="countryCode_en_d" id="countryCode_en_d" class="form-control countryCode_en_d ips hidden" >
            ...
            <div class="col-md-9"><input type="text" class="form-control" id="sh_email" name="sh_email"></div>
            <div class="col-md-3">
            <button class="but_sharess bt_email hidden" onClick="share_to('email');">Share</button>
            <button class="but_sharess bt_whatsapp hidden" onClick="share_to('whatsapp');">Share</button>
            </div>
            
            <div class="col-md-12">
            <div class="text-center">
        	<div class="correct c2"><i class="fa fa-check-circle" aria-hidden="true"></i> Sended</div>
            <div class="incorrect ic2"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> Please try again</div>
            </div>
        </div>
...
function share(way)
{
	//$(".box_share").fadeIn(300);
	//alert(1);
	if(way=='email')
	{
		$("#sh_email").prop('type','email');
		$(".box_share,.box_share_bg").fadeIn(300);
		$("#sh_email").focus();
		$(".textsh").text('Email');
		$(".bt_email").removeClass('hidden');
	}
	else if(way=='whatsapp')
	{
		$("#sh_email").prop('type','number');
		$(".box_share,.box_share_bg").fadeIn(300);
		$("#sh_email").focus();
		$(".textsh").text('Phone No');
		$(".bt_whatsapp,.countryCode_en_d").removeClass('hidden');
	}
	else if(way=='line')
	{
		var ylink = $(".txs_link").val();
		var url = 'https://social-plugins.line.me/lineit/share?url='+ylink;
		window.open(url,'_blank');
	}
}

function share_to(way)
{
	if(way=='email')
	{
		if($("#sh_email").val()=='')
		...
		{
			$.ajax({
				url:"<?php echo $url_link;?>libs/actions/send-sharing-email.php",
				type:"POST",
				dataType:"json",
				data:{
					id:$(".txs_id").val(),
	...
	else if(way=='whatsapp')
	{
		...
		{
			var Phone = $("#countryCode_en_d").val()+$("#sh_email").val();
			var yname = $(".txs_name").val();
			var yshortdescription = $(".txs_shortdescription").val();
			var ylink = $(".txs_link").val();
			var urll = 'https://api.whatsapp.com/send?phone='+Phone+'&text='+yshortdescription+'  '+ylink;//+yname+' \n\r '
...
{
	$(".box_share_bg").fadeOut(300);
	$(".box_share").fadeOut(300);
	$(".but_sharess,.countryCode_en_d").addClass('hidden');
}
function closepopup()
{
	$(".popback").fadeOut(300);
...
function open_popup_2(id)
{
	...
	$.ajax({
		url:"<?php echo $url_link;?>libs/actions/set-recent-yacht.php",
		type:"POST",
		dataType:"json",
		data:{id:id},
		success: function(res){
	...
	$.ajax({
		...
		success: function(res){
			...
			$(".dl_hires").attr('href',res.photo_8);//photo_9
			$(".txs_link").val(res.links);
			$(".txs_name").val(res.name);
			$(".txs_shortdescription").val(res.shortdescription);
			$(".txs_id").val(res.id);
		}
	});
}
