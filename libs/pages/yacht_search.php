            ...
            <div class="col-sm-12 col-md-8 col-lg-9 ">
                <div class="row1">
                    ...
                    <div class="col-md-3 col-sm-12 col-xs-12 t_t23 nopad_768">
                        <select id="cbbMarina" name="cbbMarina" class="cbb_marina sel_yacht  top7"><!-- onChange="load_subdestination(this);"-->
                            <option value="all"  <?php echo isset($_REQUEST['marina'])?'':'selected';?>>All Marina location - optional  </option>
                            <?php 
                            $sql_marina = $dbc->Query("select * from yacth_marina where status > 0 order by name asc");
                            while($marina = $dbc->Fetch($sql_marina))
                            {
								$act = ($_REQUEST['marina']==$marina['slug'])?'selected':'';
                    ...
                    <div class="col-md-3 col-sm-12 col-xs-12 t_t23 nopad_768">
                        <select id="cbbType" name="cbbType" class="cbb_type sel_yacht  top7"><!-- onChange="load_subdestination(this);"-->
                ...
                <div class="col-md-9 col-xs-12 left25" style="padding-top: 10px;">
                    <div class="ztop">
                        <div class="col-md-4 col-xs-4 col-sm-4">
                            <select id="cbbCharter" name="cbbCharter"  class="pull-right form-control cbbox ontop"><!--onChange="load_subdestination(this);"-->
                                <option value="all"  <?php echo isset($_REQUEST['charter'])?'':'selected';?>>All Type of charter  </option>
                                <?php 
                                $sql_marina = $dbc->Query("select * from yacth_charter where status > 0 order by name asc");
                                while($marina = $dbc->Fetch($sql_marina))
                                {
                                    $act = ($_REQUEST['charter']==$marina['id'])?'selected':'';
                                    echo '<option class="mar '.$marina['destination'].'" value="'.$marina['id'].'" '.$act.'>'.$marina['name'].'</option>';
                                }
                                ?>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4 col-xs-4 col-sm-4">
                            <select id="cbbCabin" name="cbbCabin"  class="pull-right form-control cbbox ontop"><!--onChange="load_subdestination(this);"-->
                                <option value="all" <?php echo isset($_REQUEST['cabin'])?'':'selected';?>>All No. of Cabin</option>
                                <option value="1-4" <?php echo ($_REQUEST['cabin']=='1-4')?'selected':'';?>>1-4 Bedroom</option>
                                <option value="5-7" <?php echo ($_REQUEST['cabin']=='5-7')?'selected':'';?>>5-7 Bedroom</option>
                                <option value="more" <?php echo ($_REQUEST['cabin']=='more')?'selected':'';?>>8&gt; Bedroom</option> 
                            </select>
                        </div>
                        
                        <div class="col-md-4 col-xs-4 col-sm-4">
                            <select name="cbbPrice" id="cbbPrice" class="pull-right form-control cbbox ontop">
                                <option value="all"  <?php echo isset($_REQUEST['price'])?'':'selected';?>>All Price Range</option>
                                <option value="under-1000USD" <?php echo ($_REQUEST['price']=='under-1000USD')?'selected':'';?>>Under 1,000USD</option>
                                <option value="1000-2000USD" <?php echo ($_REQUEST['price']=='1000-2000USD')?'selected':'';?>>1,000USD-2,000USD</option>
                                <option value="2000-3000USD" <?php echo ($_REQUEST['price']=='2000-3000USD')?'selected':'';?>>2,000USD-3,000USD</option>
...
function searching_yacht()
{
	...
	var desti = vallu[1];
	var marina = $("#cbbMarina").val();
	var type = $("#cbbType").val();
	var charter = $("#cbbCharter").val();
	var cabin = $("#cbbCabin").val();
	var price = $("#cbbPrice").val();
	window.location = "<?php echo $url_link;?>search-yacht_"+desti+'_'+marina+'_'+type+'_'+charter+'_'+cabin+'_'+price+'.html';//+'-'+type+'-'+price+'.html';
}
</script>