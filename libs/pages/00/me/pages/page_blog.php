					...
					while($row = $dbc->Fetch($sql))
					{
						...
						echo '<article class="mg-post">';
							...
							if($row['id']==7)
							{
								...
								$link = "http://www.inspiringvillas.com//lifestyle/the-10-best-phuket-luxury-villas.html";
							}
							else
							{
								$urll = "/Lifestyle-".$rid1.".html";
								$link = "http://www.inspiringvillas.com/Lifestyle-'".$row['id'].".html'";
							}
								...
								echo '<div class="mg-post-meta">';
                                    ...
                                    <ul class="mg-footer-social btnn pull-right">
                                        <li data-toggle="tooltip" data-placement="bottom" title="Share" onClick="onShare('<?php echo $row['id'];?>','<?php echo $row['name'];?>','<?php echo base64_decode($row['brief'],true);?>','<?php echo imagePath($photo[0]);?>');"><a ><i class="fa fa-facebook tw"></i></a></li>
                                        <li data-toggle="tooltip" data-placement="bottom" title="Share" onclick="gPlus('<?php echo urlencode($link);?>');"><a ><i class="fa fa-google-plus tw"></i></a></li>
                    ...
                    <aside class="mg-widget">
						...
						while($line = $dbc->Fetch($sql_rec))
						{
							...
							else
							{
								...
								$link = "http://www.inspiringvillas.com/Lifestyle-'".$line['id'].".html'";
							}
							echo '<li>';
                                echo '<div class="mg-recnt-post">';
                                    echo '<div class="mg-rp-date">'.day($line['day']).'<div class="mg-rp-month">'.month($line['updated']).'</div></div>';
  ...
  (function(d, s, id){
     ...
     fjs.parentNode.insertBefore(js, fjs);
  		...
  		FB.ui({
 			  method: 'feed',
			  name: title,
			  link: 'http://www.inspiringvillas.com/?page=blogdetail&id='+idp,
			  caption: title,
			  display: 'popup',
			  description: desc+'...',
