<?php
$parent_7 = "parent ='".$_REQUEST['des']."' and";
$cate_7 = "";
	
	//-- get beach name

$s_sql = $dbc->Query("select * from destinations where ".$parent_7."  status > 0");
while($line = $dbc->Fetch($s_sql))
{
	$bea_name = explode(",",$line['name']);
	$arr_beach_name_7[] = array(
		'id'=> $line['id'],
		'name'=> $bea_name[0],
		'parent'=> $line['parent']
	);
...
//-- get collection name
$c_sql = $dbc->Query("select * from categories where ".$cate_7." status > 0");
while($line_c = $dbc->Fetch($c_sql))
{
	$col_name = explode(" V",$line_c['name']);
	$arr_coll_name_7[] = array(
		'id'=> $line_c['id'],
		'name'=> $col_name[0]
	);
}
...
//print_r($arr_beach_name_7);

foreach($arr_beach_name_7 as $abn)
{
	if($abn['parent']=='')
	{
		if(check_links_footer($abn['id'],$abn['id'],0,0)==true)
		{
			...
			array_push($arr_link_7,$alink);
			$xx7++;
		}
	}
	else
	{
		if(check_links_footer($abn['parent'],$abn['id'],0,0)==true)
		{
