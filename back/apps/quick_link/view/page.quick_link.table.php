...
<script>
  /*$(function() {
	$("#panel-heading-group").append('<button id="btnSavesort" type="button" onclick="fn.app.quick_link.quick_link.sort()" class="btn btn-info pull-right">Save Sort</button>');
	$( "#tblSlide" ).children().sortable();
	$( "#tblSlide " ).disableSelection();
  });


fn.app.quick_link.quick_link.sort = function(id) {
	var data = {
		tables : []			
	};
	var io=1;
	...
	$.ajax({
		url:"apps/quick_link/xhr/action-save-sort.php",
		type:"POST",
		dataType:"json"	,
		data:data,
...
$(function(){
	...
	$("#tblSlide").DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "apps/quick_link/store/store-quick_link.php",
		"aoColumns": [
			{ "bSortable": true},
		...
		"createdRow": function ( row, data, index ) {
			...
			s = '';
			s += fn.engine.datatable.button('btn-default','fa-pencil','fn.app.quick_link.quick_link.change('+data[0]+')','Edit',' ');
			...
			var p='';
			/*var li = 'https://staging.inspiringvillas.com';
			var li = 'http://127.0.0.1:8131';*/
			var li = '';
			p+= data[1];
			p+= '<input type="hidden" name="tid" value="'+data[0]+'">';
			p+= '<input type="hidden" name="sor" value="'+ii+'"><br>';
			p += '<a href="'+li+data[1]+'" target="_blank"><button type=button class="btn btn-info" ><i class="fa fa-search"></i></button></a>';
			$('td', row).eq(1).html(p);
			ii ++;
