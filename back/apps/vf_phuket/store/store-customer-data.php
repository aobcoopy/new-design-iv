<?php
	...
	$sTable = "villa_form_mapping AS vfm 
	LEFT JOIN properties AS p ON vfm.villa = p.id
	LEFT JOIN users AS u ON vfm.user = u.id";
	$sLimit = "";
	...
	$sOrder = " ORDER BY vfm.id DESC";


	$sWhere = "";
	if ( $_REQUEST['search']['value'] != "" ){
		$sWhere = "WHERE vfm.villa = '".$_REQUEST['pid']."' AND (";//---------where option
	...
	{
		$sWhere .="WHERE vfm.villa = '".$_REQUEST['pid']."' ";//---------where option
	}
	...
	while ( $aRow = $dbc->Fetch( $rResult ) ){
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ ){
			...
			elseif($i==8)
			{
				$spl_data = $dbc->GetRecord("shopping_list","*","villa_form_mapping = '".$aRow[$i]."'");
				$row[] = $spl_data['status'];
			}
			elseif($i==9)
			{
