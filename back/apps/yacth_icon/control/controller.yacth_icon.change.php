...
<script tyle="text/javascript">
	fn.app.yacth_destination.yacth_destination.change = function(id) {
		$.ajax({
			url: "apps/yacth_destination/view/dialog.yacth_destination.edit.php",
			data: {id:id},
			type: "POST",
			dataType: "html",
	...
	fn.app.yacth_destination.yacth_destination.save_change = function(){
		...
		{ 
			$.post('apps/yacth_destination/xhr/action-edit-yacth_destination.php',$('#form_edit_slide').serialize(),function(response){
				if(response.success){
					$("#tblSlide").DataTable().draw();
					$("#dialog_edit_group").modal('hide');
					//$("#thumbnail_edit").attr('src','../../../../upload/yacth_destination.jpg');
				}else{
	...
	fn.app.yacth_destination.yacth_destination.upload_photo_edit = function(){
	...
	fn.app.yacth_destination.yacth_destination.upload_photo_file_edit = function(){
		var data = new FormData($("#form_edit_photo")[0]);
		var s = '';
		if($("#txt_photo_edit").val() != ''){
			remove_photo_file($("#txt_photo_edit").val());
		}
		var url = 'apps/yacth_destination/xhr/action-up-photo.php?format_photo_name='+format_photo($("#txTitle_edit").val());
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
...
/*	fn.app.yacth_destination.yacth_destination.remove_photo_edit = function(me){
	...
	fn.app.yacth_destination.yacth_destination.upload_coverphoto_edit = function(){
	...
	fn.app.yacth_destination.yacth_destination.upload_coverphoto_file_edit = function(){
