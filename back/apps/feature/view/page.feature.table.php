...
<script>
  $(function() {
	//$("#panel-heading-group").append('<button id="btnSavesort" type="button" onclick="fn.app.feature.feature.sort()" class="btn btn-info pull-right">Save Sort</button>');
	//$( "#tblSlide" ).children().sortable();
	//$( "#tblSlide " ).disableSelection();
  });


fn.app.feature.feature.sort = function(id) {
	var data = {
		tables : []			
	};
	var io=1;
	...
	$.ajax({
		url:"apps/feature/xhr/action-save-sort.php",
		type:"POST",
		dataType:"json"	,
		data:data,
...
fn.app.edit.change_group2 = function(me,id){
		//alert(id+'----'+$(me).val());
		$.ajax({
				url: "apps/feature/xhr/action-change-group.php",
				type: "POST",
				dataType:"json",
...
$(function(){
	...
	$("#tblSlide").DataTable({
		...
		"serverSide": true,
		"stateSave": true,
		"ajax": "apps/feature/store/store-feature.php",
		"aoColumns": [
			{ "bSortable": false},
		...
		"createdRow": function ( row, data, index ) {
			...
			s = '';
			s += fn.engine.datatable.button('btn-default','fa-pencil','fn.app.feature.feature.change('+data[0]+')');
			s += ' ';
			//s += fn.engine.datatable.button('btn-warning','fa-lock','fn.app.feature.group.permission('+data[0]+')');
			$('td', row).eq(7).html(s);
