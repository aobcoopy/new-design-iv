<?php
	include_once "apps/feature/view/dialog.feature.add.php";
?>
<script style="text/javascript">

	$("#panel-heading-group").append('<button id="btnAddGroup" type="button" class="btn btn-primary pull-right">Add</button>');
	$("#btnAddGroup").click(function(){
		$("#dialog_add_feature").modal('show');
	});
	$('#dialog_add_feature').on('shown.bs.modal', function () {
		$("#txtName").focus();
	});
	
	
	fn.app.feature.feature.add = function(){
		...
		{
			$.post('apps/feature/xhr/action-add-feature.php',$('#form_add_feature').serialize(),function(response){
			if(response.success){
				$("#tblSlide").DataTable().draw();
				$("#dialog_add_feature").modal('hide');
				$("#form_add_feature")[0].reset();
				$("#path_photo").val('');
				$("#txt_photo").val('');
				$(".phos").attr('src','');
				$(".bc").hide();
				//$("#thumbnail").attr('src','../../../../upload/feature.jpg');
			}else{
	...
	fn.app.feature.feature.upload_photo = function(){
	...
	fn.app.feature.feature.upload_photo_file = function(){
		...
		var url = 'apps/feature/xhr/action-up-photo.php?format_photo_name='+new_path_name;
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
	...
	fn.app.feature.feature.remove_photo = function(me){
