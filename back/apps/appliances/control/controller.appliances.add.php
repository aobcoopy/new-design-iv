<?php
	include_once "apps/appliances/view/dialog.appliances.add.php";
?>
<script style="text/javascript">

	$("#panel-heading-group").append('<button id="btnAddGroup" type="button" class="btn btn-primary pull-right">Add</button>');
	$("#btnAddGroup").click(function(){
		$("#dialog_add_appliances").modal('show');
	});
	$('#dialog_add_appliances').on('shown.bs.modal', function () {
		$("#txtName").focus();
	});
	
	
	fn.app.appliances.appliances.add = function(){
		...
		{
			$.post('apps/appliances/xhr/action-add-appliances.php',$('#form_add_appliances').serialize(),function(response){
			if(response.success){
				$("#tblSlide").DataTable().draw();
				$("#dialog_add_appliances").modal('hide');
				$("#form_add_appliances")[0].reset();
				$("#path_photo").val('');
				$("#txt_photo").val('');
				$(".phos").attr('src','');
				$(".bc").hide();
				//$("#thumbnail").attr('src','../../../../upload/appliances.jpg');
			}else{
	...
	fn.app.appliances.appliances.upload_photo = function(){
	...
	fn.app.appliances.appliances.upload_photo_file = function(){
		...
		var url = 'apps/appliances/xhr/action-up-photo.php?format_photo_name='+new_path_name;
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
	...
	fn.app.appliances.appliances.remove_photo = function(me){
