...
<script>
  $(function() {
	//$("#panel-heading-group").append('<button id="btnSavesort" type="button" onclick="fn.app.appliances.appliances.sort()" class="btn btn-info pull-right">Save Sort</button>');
	//$( "#tblSlide" ).children().sortable();
	//$( "#tblSlide " ).disableSelection();
  });


fn.app.appliances.appliances.sort = function(id) {
	var data = {
		tables : []			
	};
	var io=1;
	...
	$.ajax({
		url:"apps/appliances/xhr/action-save-sort.php",
		type:"POST",
		dataType:"json"	,
		data:data,
...
$(function(){
	...
	$("#tblSlide").DataTable({
		...
		"serverSide": true,
		"stateSave": true,
		"ajax": "apps/appliances/store/store-appliances.php",
		"aoColumns": [
			{ "bSortable": false},
		...
		"createdRow": function ( row, data, index ) {
			...
			s = '';
			s += fn.engine.datatable.button('btn-default','fa-pencil','fn.app.appliances.appliances.change('+data[0]+')');
			s += ' ';
			//s += fn.engine.datatable.button('btn-warning','fa-lock','fn.app.appliances.group.permission('+data[0]+')');
			$('td', row).eq(7).html(s);
