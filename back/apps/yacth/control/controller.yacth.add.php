<?php
	include_once "apps/yacth/view/dialog.yacth.add.php";
?>
<script style="text/javascript">

	$("#panel-heading-group").append('<button id="btnAddGroup" type="button" class="btn btn-primary pull-right">Add</button>');
	$("#btnAddGroup").click(function(){
		$("#dialog_add_yacth").modal('show');
	});
	$('#dialog_add_yacth').on('shown.bs.modal', function () {
		$("#txtName").focus();
	});
	
	
	fn.app.yacth.yacth.add = function(){
		...
		{
			$.post('apps/yacth/xhr/action-add-yacth.php',$('#form_add_yacth').serialize(),function(response){
			if(response.success){
				$("#tblSlide").DataTable().draw();
				$("#dialog_add_yacth").modal('hide');
				$("#form_add_yacth")[0].reset();
				/*$("#path_photo").val('');
				$("#txt_photo").val('');
				$(".phos").attr('src','');
				$(".bc").hide();*/
				//$("#thumbnail").attr('src','../../../../upload/yacth.jpg');
			}else{
	...
	fn.app.yacth.yacth.upload_photo = function(){
	...
	fn.app.yacth.yacth.upload_photo_file = function(){
		...
		var url = 'apps/yacth/xhr/action-up-photo.php?format_photo_name='+new_path_name;
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
	...
	/*fn.app.yacth.yacth.remove_photo = function(me){
	...
	fn.app.yacth.yacth.upload_photo_cover = function(){
	...
	fn.app.yacth.yacth.upload_photo_file_cover = function(){
	...
	/*fn.app.yacth.yacth.remove_photo_cover = function(me){
