<?php
	...
	while ( $aRow = $dbc->Fetch( $rResult ) ){
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ ){
			...
			elseif($i==6 || $i==7)
			{
				$data = $dbc->GetRecord("yacth_destination","*","id='".$aRow[$i]."'");
				$row[] = $data['name'];
			}
			elseif($i==8)
			{
				if($dbc->GetRecord("yacth_sailing_route","*","id='".$aRow[$i]."'"))
				{
					$data = $dbc->GetRecord("yacth_sailing_route","*","id='".$aRow[$i]."'");
					$row[] = $data['name'];
				}
				else
				{
			...
			elseif($i==9)
			{
				if($data = $dbc->GetRecord("users","*","id='".$aRow[$i]."'"))
				{
					$data = $dbc->GetRecord("users","*","id='".$aRow[$i]."'");
					$row[] = $data['name'];
				}
				else
				{
