...
<script>
  $(function() {
	//$("#panel-heading-group").append('<button id="btnSavesort" type="button" onclick="fn.app.invoice.invoice.sort()" class="btn btn-info pull-right">Save Sort</button>');
	//$( "#tblSlide" ).children().sortable();
	//$( "#tblSlide " ).disableSelection();
  });


fn.app.invoice.invoice.sort = function(id) {
	var data = {
		tables : []			
	};
	var io=1;
	...
	$.ajax({
		url:"apps/invoice/xhr/action-save-sort.php",
		type:"POST",
		dataType:"json"	,
		data:data,
...
$(function(){
	...
	$("#tblSlide").DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "apps/invoice/store/store-invoice.php",
		"aoColumns": [
			{ "bSortable": false},
		...
		"createdRow": function ( row, data, index ) {
			...
			s = '';
			s += fn.engine.datatable.button('btn-default','fa-pencil','fn.app.invoice.invoice.change('+data[0]+')');
			s += ' ';
			//s += fn.engine.datatable.button('btn-warning','fa-lock','fn.app.invoice.group.permission('+data[0]+')');
			$('td', row).eq(5).html(s);
