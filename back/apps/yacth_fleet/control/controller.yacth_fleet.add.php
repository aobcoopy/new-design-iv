<?php
	include_once "apps/yacth_fleet/view/dialog.yacth_fleet.add.php";
?>
<script style="text/javascript">

	$("#panel-heading-group").append('<button id="btnAddGroup" type="button" class="btn btn-primary pull-right">Add</button>');
	$("#btnAddGroup").click(function(){
		$("#dialog_add_yacth_fleet").modal('show');
	});
	$('#dialog_add_yacth_fleet').on('shown.bs.modal', function () {
		$("#txtName").focus();
	});
	
	
	fn.app.yacth_fleet.yacth_fleet.add = function(){
		...
		{
			$.post('apps/yacth_fleet/xhr/action-add-yacth_fleet.php',$('#form_add_yacth_fleet').serialize(),function(response){
			if(response.success){
				$("#tblSlide").DataTable().draw();
				$("#dialog_add_yacth_fleet").modal('hide');
				$("#form_add_yacth_fleet")[0].reset();
				/*$("#path_photo").val('');
				$("#txt_photo").val('');
				$(".phos").attr('src','');
				$(".bc").hide();*/
				//$("#thumbnail").attr('src','../../../../upload/yacth_fleet.jpg');
			}else{
	...
	fn.app.yacth_fleet.yacth_fleet.upload_photo = function(){
	...
	fn.app.yacth_fleet.yacth_fleet.upload_photo_file = function(){
		...
		var url = 'apps/yacth_fleet/xhr/action-up-photo.php?format_photo_name='+new_path_name;
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
	...
	/*fn.app.yacth_fleet.yacth_fleet.remove_photo = function(me){
		var file_path = $(me).parent().find('.paths').val();
		//alert(file_path);
		$.ajax({
			url:"apps/yacth_fleet/xhr/action-remove-photo.php",
			type:"POST",
			dataType:"json",
			data:{path:file_path},
	...
	fn.app.yacth_fleet.yacth_fleet.upload_photo_cover = function(){
	...
	fn.app.yacth_fleet.yacth_fleet.upload_photo_file_cover = function(){
	...
	/*fn.app.yacth_fleet.yacth_fleet.remove_photo_cover = function(me){
