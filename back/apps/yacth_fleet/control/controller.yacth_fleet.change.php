...
<script tyle="text/javascript">
	fn.app.yacth_fleet.yacth_fleet.change = function(id) {
		$.ajax({
			url: "apps/yacth_fleet/view/dialog.yacth_fleet.edit.php",
			data: {id:id},
			type: "POST",
			dataType: "html",
	...
	fn.app.yacth_fleet.yacth_fleet.save_change = function(){
		...
		else if(document.getElementById("txLinke_edit").value==''  )
		{
			alert('The Title, Short Title, Filter name and URL Slug are required');
			$("#txLinke_edit").focus();
			return false;
		...
		{ 
			$.post('apps/yacth_fleet/xhr/action-edit-yacth_fleet.php',$('#form_edit_slide').serialize(),function(response){
				if(response.success){
					$("#tblSlide").DataTable().draw();
					$("#dialog_edit_group").modal('hide');
					//$("#thumbnail_edit").attr('src','../../../../upload/yacth_fleet.jpg');
				}else{
	...
	fn.app.yacth_fleet.yacth_fleet.upload_photo_edit = function(){
	...
	fn.app.yacth_fleet.yacth_fleet.upload_photo_file_edit = function(){
		var data = new FormData($("#form_edit_photo")[0]);
		var s = '';
		if($("#txt_photo_edit").val() != ''){
			remove_photo_file($("#txt_photo_edit").val());
		}
		var url = 'apps/yacth_fleet/xhr/action-up-photo.php?format_photo_name='+format_photo($("#txTitle_edit").val());
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
...
/*	fn.app.yacth_fleet.yacth_fleet.remove_photo_edit = function(me){
	...
	fn.app.yacth_fleet.yacth_fleet.upload_coverphoto_edit = function(){
	...
	fn.app.yacth_fleet.yacth_fleet.upload_coverphoto_file_edit = function(){
