...
<script tyle="text/javascript">
	fn.app.wine_list.wine_list.change = function(id) {
		$.ajax({
			url: "apps/wine_list/view/dialog.wine_list.edit.php",
			data: {id:id},
			type: "POST",
			dataType: "html",
	...
	fn.app.wine_list.wine_list.save_change = function(){
		...
		{ 
			$.post('apps/wine_list/xhr/action-edit-wine_list.php',$('#form_edit_slide').serialize(),function(response){
				if(response.success){
					$("#tblSlide").DataTable().draw();
					$("#dialog_edit_group").modal('hide');
					//$("#thumbnail_edit").attr('src','../../../../upload/wine_list.jpg');
				}else{
	...
	fn.app.wine_list.wine_list.upload_photo_edit = function(){
	...
	fn.app.wine_list.wine_list.upload_photo_file_edit = function(){
		var data = new FormData($("#form_edit_photo")[0]);
		var s = '';
		if($("#txt_photo_edit").val() != ''){
			remove_photo_file($("#txt_photo_edit").val());
		}
		var url = 'apps/wine_list/xhr/action-up-photo.php?format_photo_name='+format_photo($("#txTitle_edit").val());
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
...
/*	fn.app.wine_list.wine_list.remove_photo_edit = function(me){
	...
	fn.app.wine_list.wine_list.upload_coverphoto_edit = function(){
	...
	fn.app.wine_list.wine_list.upload_coverphoto_file_edit = function(){
