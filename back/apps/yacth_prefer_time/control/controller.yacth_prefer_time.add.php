<?php
	include_once "apps/yacth_prefer_time/view/dialog.yacth_prefer_time.add.php";
?>
<script style="text/javascript">

	$("#panel-heading-group").append('<button id="btnAddGroup" type="button" class="btn btn-primary pull-right">Add</button>');
	$("#btnAddGroup").click(function(){
		$("#dialog_add_yacth_prefer_time").modal('show');
	});
	$('#dialog_add_yacth_prefer_time').on('shown.bs.modal', function () {
		$("#txTitle").focus();
	});
	
	
	fn.app.yacth_prefer_time.yacth_prefer_time.add = function(){
		if(document.getElementById("txTitle").value=='' )
		{
			alert('The Time are required');
			$("#txTitle").focus();
			return false;
		}
		...
		{
			$.post('apps/yacth_prefer_time/xhr/action-add-yacth_prefer_time.php',$('#form_add_yacth_prefer_time').serialize(),function(response){
			if(response.success){
				$("#tblSlide").DataTable().draw();
				$("#dialog_add_yacth_prefer_time").modal('hide');
				$("#form_add_yacth_prefer_time")[0].reset();
				/*$("#path_photo").val('');
				$("#txt_photo").val('');
				$(".phos").attr('src','');
				$(".bc").hide();*/
				//$("#thumbnail").attr('src','../../../../upload/yacth_prefer_time.jpg');
			}else{
	...
	fn.app.yacth_prefer_time.yacth_prefer_time.upload_photo = function(){
	...
	fn.app.yacth_prefer_time.yacth_prefer_time.upload_photo_file = function(){
		...
		var url = 'apps/yacth_prefer_time/xhr/action-up-photo.php?format_photo_name='+new_path_name;
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
	...
	/*fn.app.yacth_prefer_time.yacth_prefer_time.remove_photo = function(me){
		var file_path = $(me).parent().find('.paths').val();
		//alert(file_path);
		$.ajax({
			url:"apps/yacth_prefer_time/xhr/action-remove-photo.php",
			type:"POST",
			dataType:"json",
			data:{path:file_path},
	...
	fn.app.yacth_prefer_time.yacth_prefer_time.upload_photo_cover = function(){
	...
	fn.app.yacth_prefer_time.yacth_prefer_time.upload_photo_file_cover = function(){
	...
	/*fn.app.yacth_prefer_time.yacth_prefer_time.remove_photo_cover = function(me){
