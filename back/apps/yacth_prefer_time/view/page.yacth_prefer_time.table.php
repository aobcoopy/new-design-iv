<div class="col-md-12">
	<div class="panel panel-default">
		<div id="panel-heading-group" class="panel-heading">
	        <h3 class="panel-title">Yacht Prefer Time</h3>
	    </div>
...
<script>
 /* $(function() {
	$("#panel-heading-group").append('<button id="btnSavesort" type="button" onclick="fn.app.yacth_prefer_time.yacth_prefer_time.sort()" class="btn btn-info pull-right">Save Sort</button>');
	$( "#tblSlide" ).children().sortable();
	$( "#tblSlide " ).disableSelection();
  });*/


fn.app.yacth_prefer_time.yacth_prefer_time.sort = function(id) {
	var data = {
		tables : []			
	};
	var io=1;
	...
	$.ajax({
		url:"apps/yacth_prefer_time/xhr/action-save-sort.php",
		type:"POST",
		dataType:"json"	,
		data:data,
...
$(function(){
	...
	$("#tblSlide").DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "apps/yacth_prefer_time/store/store-yacth_prefer_time.php",
		"aoColumns": [
			{ "bSortable": false},
		...
		"createdRow": function ( row, data, index ) {
			...
			s = '';
			s += fn.engine.datatable.button('btn-default','fa-pencil','fn.app.yacth_prefer_time.yacth_prefer_time.change('+data[0]+')');
			s += ' ';
			//s += fn.engine.datatable.button('btn-warning','fa-lock','fn.app.yacth_prefer_time.group.permission('+data[0]+')');
			$('td', row).eq(4).html(s);
