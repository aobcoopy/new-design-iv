...
$(function(){
	...
	$("#tblGroup").DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "apps/sending/store/store-sending.php",
		"aoColumns": [
			{ "bSortable": false},
			{"bSort" : true},
			{"bSortable": true},
		...
		"createdRow": function ( row, data, index ) {
			...
			s = '';
			s += fn.engine.datatable.button('btn-default','fa-search','fn.app.sending.sending.change('+data[0]+')');
			s += ' ';
			s += fn.engine.datatable.button('btn-info','fa-tag','fn.app.sending.sending.save_change('+data[0]+')');
			$('td', row).eq(11).html(s);
			...
			if(data[9]==4)
			{
				t += '<button class="btn btn-xs btn-success"><i class="fa fa-truck " aria-hidden="true"></i> sending</button>';
			}
			else
			{
			}
