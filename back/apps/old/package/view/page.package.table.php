...
$(function(){
	...
	$("#tblGroup").DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "apps/package/store/store-package.php",
		"aoColumns": [
			{ "bSortable": false},
			{"bSort" : true},
			{"bSortable": true},
		...
		"createdRow": function ( row, data, index ) {
			...
			s = '';
			s += fn.engine.datatable.button('btn-default','fa-search','fn.app.package.package.change('+data[0]+')');
			s += ' ';
			s += fn.engine.datatable.button('btn-info','fa-truck ','fn.app.package.package.save_change('+data[0]+')');
			s += ' ';
			s += fn.engine.datatable.button('btn-warning','fa-print  ','fn.app.package.package.print('+data[0]+')');
			$('td', row).eq(10).html(s);
			...
			if(data[9]==3)
			{
				t += '<button class="btn btn-xs btn-info"><i class="fa fa-cubes" aria-hidden="true"></i> package</button>';
			}
			else
			{
			}
