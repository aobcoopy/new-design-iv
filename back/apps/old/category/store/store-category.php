<?php
	...
	$sTable = "categories where parent IS NULL  ORDER BY parent DESC";
	$sLimit = "";
	...
	while ( $aRow = $dbc->Fetch( $rResult ) ){
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ ){
			...
			elseif($i==4)
			{
				$ro = $dbc->GetRecord("categories","*","id='".$aRow[$i]."'");
				if($ro['parent']=='NULL')
				{
				...
				{
					$but = array();
					$sql = $dbc->Query("select * from categories where parent = '".$aRow[$i]."' ");
