<?php
	...
	$main = array();
	if(isset($_REQUEST['tx_main_e']))//------What is include
	{
		foreach($_REQUEST['tx_main_e'] as $tx_main)
		{
			$ads = str_replace("'","#",$tx_main);
			array_push($main,$ads);
		}
	}
	
	$transfer = array();
	if(isset($_REQUEST['tx_transfer_e']))//------Complimentary airport transfer
	{
		foreach($_REQUEST['tx_transfer_e'] as $tx_transfer_e)
		{
			$ads = str_replace("'","#",$tx_transfer_e);
			array_push($transfer,$ads);
		}
	}
	
	$service = array();
	if(isset($_REQUEST['tx_service_e']))//------Staff service inclusion
	{
		foreach($_REQUEST['tx_service_e'] as $tx_service_e)
		{
			$ads = str_replace("'","#",$tx_service_e);
			array_push($service,$ads);
		}
	}
	
	$Charge = array();
	if(isset($_REQUEST['tx_Charge_e']))//------Extra Charge
	{
		foreach($_REQUEST['tx_Charge_e'] as $tx_Charge_e)
		{
			$ads = str_replace("'","#",$tx_Charge_e);
			array_push($Charge,$ads);
		}
	}
	
	$Special = array();
	if(isset($_REQUEST['tx_Special_e']))//------Special Note
	{
		foreach($_REQUEST['tx_Special_e'] as $tx_Special_e)
		{//addslashes()
			$ads = str_replace("'","#",$tx_Special_e);
			//echo $ads.'<br>';
			array_push($Special,$ads);
		}
	}
	//print_r($Special);
	$available = array();
	if(isset($_REQUEST['tx_available_e']))//------Available at extra charge
	{
		foreach($_REQUEST['tx_available_e'] as $tx_available_e)
		{
			$ads = str_replace("'","#",$tx_available_e);
			array_push($available,$ads);
		}
	}
	
	$House = array();
	if(isset($_REQUEST['tx_House_e']))//------House rules
	{
		foreach($_REQUEST['tx_House_e'] as $tx_House_e)
		{
			$ads = str_replace("'","#",$tx_House_e);
			array_push($House,$ads);
		}
	}
	// facility
	$fac = '';//array();
