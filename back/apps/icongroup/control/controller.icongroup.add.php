<?php
	include_once "apps/icongroup/view/dialog.icongroup.add.php";
?>
<script style="text/javascript">

	$("#panel-heading-group").append('<button id="btnAddGroup" type="button" class="btn btn-primary pull-right">Add</button>');
	$("#btnAddGroup").click(function(){
		$("#dialog_add_icongroup").modal('show');
	});
	$('#dialog_add_icongroup').on('shown.bs.modal', function () {
		$("#txtName").focus();
	});
	
	
	fn.app.icongroup.icongroup.add = function(){
		...
		{
			$.post('apps/icongroup/xhr/action-add-icongroup.php',$('#form_add_icongroup').serialize(),function(response){
			if(response.success){
				$("#tblSlide").DataTable().draw();
				$("#dialog_add_icongroup").modal('hide');
				$("#form_add_icongroup")[0].reset();
				$("#path_photo").val('');
				$("#txt_photo").val('');
				$(".phos").attr('src','');
				$(".bc").hide();
				//$("#thumbnail").attr('src','../../../../upload/icongroup.jpg');
			}else{
	...
	fn.app.icongroup.icongroup.upload_photo = function(){
	...
	fn.app.icongroup.icongroup.upload_photo_file = function(){
		...
		var url = 'apps/icongroup/xhr/action-up-photo.php?format_photo_name='+new_path_name;
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
	...
	fn.app.icongroup.icongroup.remove_photo = function(me){
