...
<script>
  $(function() {
	$("#panel-heading-group").append('<button id="btnSavesort" type="button" onclick="fn.app.testimonial.testimonial.sort()" class="btn btn-info pull-right">Save Sort</button>');
	$( "#tblSlide" ).children().sortable();
	$( "#tblSlide " ).disableSelection();
  });


fn.app.testimonial.testimonial.sort = function(id) {
	var data = {
		tables : []			
	};
	var io=1;
	...
	$.ajax({
		url:"apps/testimonial/xhr/action-save-sort.php",
		type:"POST",
		dataType:"json"	,
		data:data,
...
$(function(){
	...
	$("#tblSlide").DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "apps/testimonial/store/store-testimonial.php",
		"aoColumns": [
			{ "bSortable": false},
		...
		"createdRow": function ( row, data, index ) {
			...
			s = '';
			s += fn.engine.datatable.button('btn-default','fa-pencil','fn.app.testimonial.testimonial.change('+data[0]+')');
			/*s += ' ';
			s += fn.engine.datatable.button('btn-info','fa-check-square-o','fn.app.testimonial.testimonial.check('+data[0]+')');
			s += ' ';
			s += fn.engine.datatable.button('btn-warning','fa-calendar','fn.app.testimonial.testimonial.popup_pricing('+data[0]+')');*/
			$('td', row).eq(7).html(s);
