<?php
	include_once "apps/yacth_charter/view/dialog.yacth_charter.add.php";
?>
<script style="text/javascript">

	$("#panel-heading-group").append('<button id="btnAddGroup" type="button" class="btn btn-primary pull-right">Add</button>');
	$("#btnAddGroup").click(function(){
		$("#dialog_add_yacth_charter").modal('show');
	});
	$('#dialog_add_yacth_charter').on('shown.bs.modal', function () {
		$("#txTitle").focus();
	});
	
	
	fn.app.yacth_charter.yacth_charter.add = function(){
		...
		{
			$.post('apps/yacth_charter/xhr/action-add-yacth_charter.php',$('#form_add_yacth_charter').serialize(),function(response){
			if(response.success){
				$("#tblSlide").DataTable().draw();
				$("#dialog_add_yacth_charter").modal('hide');
				$("#form_add_yacth_charter")[0].reset();
				/*$("#path_photo").val('');
				$("#txt_photo").val('');
				$(".phos").attr('src','');
				$(".bc").hide();*/
				//$("#thumbnail").attr('src','../../../../upload/yacth_charter.jpg');
			}else{
	...
	fn.app.yacth_charter.yacth_charter.upload_photo = function(){
	...
	fn.app.yacth_charter.yacth_charter.upload_photo_file = function(){
		...
		var url = 'apps/yacth_charter/xhr/action-up-photo.php?format_photo_name='+new_path_name;
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
	...
	/*fn.app.yacth_charter.yacth_charter.remove_photo = function(me){
	...
	fn.app.yacth_charter.yacth_charter.upload_photo_cover = function(){
	...
	fn.app.yacth_charter.yacth_charter.upload_photo_file_cover = function(){
	...
	/*fn.app.yacth_charter.yacth_charter.remove_photo_cover = function(me){
