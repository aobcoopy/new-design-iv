<?php
	...
	$aColumns = array(
		...
		'c.status',
		'c.id',
		'SUBSTRING_INDEX(c.created," ",1) AS mydate'
	);
	$sIndexColumn = "c.id";
	
	$sTable = "contact_us AS c LEFT JOIN users AS u ON c.users = u.id   ";//WHERE  c.property IS  NULL order by c.id desc
	$sLimit = "";
	...
	if ( $_REQUEST['search']['value'] != "" ){
		$sWhere = "WHERE c.property IS  NULL AND (";
	...
	{
		$sWhere .="WHERE c.property IS  NULL ";
	} 
	$sWhere .=" AND SUBSTRING_INDEX(c.created,' ',1) BETWEEN '".$_REQUEST['tx_start']."'  AND '".$_REQUEST['tx_end']."'";
