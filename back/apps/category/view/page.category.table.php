...
<script>
  $(function() {
	$("#panel-heading-group").append('<button id="btnSavesort" type="button" onclick="fn.app.category.category.sort()" class="btn btn-info pull-right">Save Sort</button>');
	$( "#tblSlide" ).children().sortable();
	$( "#tblSlide " ).disableSelection();
  });


fn.app.category.category.sort = function(id) {
	var data = {
		tables : []			
	};
	var io=1;
	...
	$.ajax({
		url:"apps/category/xhr/action-save-sort.php",
		type:"POST",
		dataType:"json"	,
		data:data,
...
$(function(){
	...
	$("#tblSlide").DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "apps/category/store/store-category.php",
		"aoColumns": [
			{ "bSortable": false},
		...
		"createdRow": function ( row, data, index ) {
			...
			s = '';
			s += fn.engine.datatable.button('btn-default','fa-pencil','fn.app.category.category.change('+data[0]+')');
			s += ' ';
			//s += fn.engine.datatable.button('btn-warning','fa-lock','fn.app.category.group.permission('+data[0]+')');
			$('td', row).eq(11).html(s);
