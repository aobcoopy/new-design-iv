...
<script tyle="text/javascript">
	fn.app.spl_items.spl_items.change = function(id) {
		$.ajax({
			url: "apps/spl_items/view/dialog.spl_items.edit.php",
			data: {id:id},
			type: "POST",
			dataType: "html",
	...
	fn.app.spl_items.spl_items.save_change = function(){
		if(document.getElementById("tx_name_edit").value=='' )
		{
			fn.app.spl_items.check('tx_name_edit');
		}
		else if(document.getElementById("tx_unit_edit").value=='')
		{
			fn.app.spl_items.check('tx_unit_edit');
		}
		else if(document.getElementById("tx_unit_type_edit").value=='')
		{
			fn.app.spl_items.check('tx_unit_type_edit');
		}
		else if(document.getElementById("tx_price_edit").value=='')
		{
			fn.app.spl_items.check('tx_price_edit');
		}
		else
		{ 
			$.post('apps/spl_items/xhr/action-edit-spl_items.php',$('#form_edit_slide').serialize(),function(response){
				if(response.success){
					//$("#tblSlide").DataTable().draw();
					$("#dialog_edit_group").modal('hide');
					fn.app.spl_items.spl_items.load_data();
					//$("#thumbnail_edit").attr('src','../../../../upload/spl_items.jpg');
				}else{
	...
	fn.app.spl_items.spl_items.upload_photo_edit = function(){
	...
	fn.app.spl_items.spl_items.upload_photo_file_edit = function(){
		var data = new FormData($("#form_edit_photo")[0]);
		var s = '';
		if($("#txt_photo_edit").val() != ''){
			remove_photo_file($("#txt_photo_edit").val());
		}
		var url = 'apps/spl_items/xhr/action-up-photo.php?format_photo_name='+format_photo($("#txTitle_edit").val());
		jQuery.ajax({
			url: url,
			data: data,
			cache: false,
...
/*	fn.app.spl_items.spl_items.remove_photo_edit = function(me){
	...
	fn.app.spl_items.spl_items.upload_coverphoto_edit = function(){
	...
	fn.app.spl_items.spl_items.upload_coverphoto_file_edit = function(){
